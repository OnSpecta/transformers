...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if calibrate:
            import torch.ao.quantization.quantizer.xnnpack_quantizer
            from torch.ao.quantization.quantize_pt2e import (
                convert_pt2e,
                prepare_pt2e)
            if past_key_values is None:
                if self.quantized_prompt_model is None:
                    self.quantized_prompt_model, _ = torch._dynamo.export(self.model, *(input_ids, attention_mask, position_ids), aten_graph=True)

                    quantizer = torch.ao.quantization.quantizer.xnnpack_quantizer.XNNPACKQuantizer()
                ...
                if self.quantized_token_model is None:
                    self.quantized_token_model, _ = torch._dynamo.export(self.model, *(input_ids, attention_mask, position_ids, past_key_values), aten_graph=True)
